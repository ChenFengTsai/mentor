...
class MetersGroup(object):
    def __init__(self, csv_file_name, formating, use_wandb):
        ...
        self._all_fieldnames = set()  # Track all fieldnames we've seen
    ...
    def _remove_old_entries(self, data):
        ...
        # Update fieldnames with all keys we've seen
        all_fieldnames = sorted(self._all_fieldnames.union(data.keys()))
    ...
    def _dump_to_csv(self, data):
        ...
        self._all_fieldnames.update(data.keys())
        ...
        if self._csv_writer is None:
            ...
            # Use all fieldnames we've seen so far
            all_fieldnames = sorted(self._all_fieldnames)
            ...
            # Check if we need to recreate the writer with new fieldnames
            current_fieldnames = set(self._csv_writer.fieldnames)
            new_fieldnames = set(data.keys())
            
            if not new_fieldnames.issubset(current_fieldnames):
                # We have new fields, need to recreate the CSV with updated headers
    ...
    def _dump_to_console(self, data, prefix):
        ...
        # Add any extra fields not in the standard format
        standard_keys = {key for key, _, _ in self._formating}
        extra_keys = set(data.keys()) - standard_keys - {'frame'}
        for key in sorted(extra_keys):
            value = data[key]
            # Default to float formatting for extra fields
            pieces.append(f'{key}: {value:.04f}' if isinstance(value, float) else f'{key}: {value}')
